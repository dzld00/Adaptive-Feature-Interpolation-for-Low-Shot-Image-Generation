...
def near_interp(embeddings, k, augment_prob):
    ...
    for idx in range(embeddings.size()[0]):
        p = random.random()
        if p < augment_prob:
            out_embeddings = torch.cat((out_embeddings, inner_embeddings[idx,:].unsqueeze(0)), dim=0)
        else:
            out_embeddings = torch.cat((out_embeddings, embeddings[idx,:].unsqueeze(0)), dim=0)
    
    samples = out_embeddings[batch_size:,:]
